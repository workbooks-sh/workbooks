// `workbook workgroup pull|push` — round-trips the broader portal
// config for a group. One canonical .mjs file holds:
...
//
// Folder structure + workbook positions are NOT in here — those are
// the dedicated `workbook group export` / `workbook group import`
// manifest surface (wb-43m). Run the two side-by-side if you want
// the full picture.
//
// The agent edits the file, runs `push`, the broker fans out to the
// same endpoints Studio uses.
...
async function pull(groupId, flags, ctx) {
  // Parallel fetch: group meta, tag dict, env declarations. Folder
  // structure + workbook positions live in `workbook group export`.
  const [group, tags, env] = await Promise.all([
...
async function push(groupId, flags, ctx) {
  ...
  // 1. Tag dictionary (additive — we don't delete unknown tags so
  ...
  // 2. Env-var declarations — names only. The broker already enforces
