...
import { loadLocalCreds } from "../util/localCreds.mjs";

export async function runDev(opts = {}) {
  const { project = ".", port, runtime } = opts;
  const config = await loadConfig(project);

  // Dev always loads workbook.local.json when present. Gitignored by
  // convention — never check it in. Empty / missing file is fine.
  const local = await loadLocalCreds(config.root);
  if (local?.databases) {
    config.databasesBaked = local.databases;
    process.stderr.write(
      `[workbook dev] loaded ${Object.keys(local.databases).length} database creds from workbook.local.json\n`,
    );
  }
