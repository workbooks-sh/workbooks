...
function extractHeadEssentials(html) {
  ...
  // wb-integrations is read by the hosted viewer (workbooks.sh) BEFORE
  // executing the workbook body so it can prompt the recipient to
  // connect each declared toolkit. Hoist it out of the compression
  // sandwich so the viewer doesn't have to decompress to discover it.
  const integrationsRe = /<script[^>]*\bid\s*=\s*["']?wb-integrations["']?[^>]*>[\s\S]*?<\/script>/i;
  // wb-databases follows the same pattern — Studio resolves group-level
  // connections from the slot list before posting bind messages. Static
  // inspectors (inspect-tools, broker indexer) also benefit.
  const databasesRe = /<script[^>]*\bid\s*=\s*["']?wb-databases["']?[^>]*>[\s\S]*?<\/script>/i;
  ...
  const m7 = head.match(integrationsRe); if (m7) out.push(m7[0]);
  const m8 = head.match(databasesRe); if (m8) out.push(m8[0]);
