 ...
 *  minimum for the workbook. "none" is the smallest — it ships zero
 *  wasm — and only fits workbooks that reference zero wasm symbols. */
const VARIANTS_ASCENDING = ["none", "app", "minimal", "default"];
 ...
 */
export async function checkVariant({ runtimeDir, variant, bundleSrc, variantExplicit = true }) {
  const warnings = [];
  ...
  // "none" exports nothing — synthesize an empty set rather than
  // reading from disk (no pkg-none/ directory exists).
  const chosenExports = variant === "none"
    ? new Set()
    : await readVariantExports(runtimeDir, variant);
  ...
  if (usedHeavy.size > 0) {
    ...
    for (const v of VARIANTS_ASCENDING) {
      ...
      if (v === "none") continue;  // "none" never covers any symbol
    ...
    if (variant === "none") {
      warnings.push(
        `wasmVariant: "none" was selected but the bundle references ${list.length} ` +
        `wasm symbol${list.length === 1 ? "" : "s"} (${preview}). ` +
        `Switch to wasmVariant: "${recommended}" to include the runtime, ` +
        `or remove the calls.`,
      );
    } else {
      warnings.push(
  ...
  // Zero heavy-symbols + variant unset → suggest "none" so the author
  // sheds the ~200-300 KB wasm-runtime layer they're not using. Only
  // fires on the implicit-default path; an explicit pick is honored.
  if (!variantExplicit && variant === "default") {
    warnings.push(
      `wasmVariant is unset and no wasm.* symbols were detected — ` +
      `consider wasmVariant: "none" to omit the runtime entirely ` +
      `(saves ~200-300 KB on the .html).`,
    );
    return { warnings };
  }
  ...
  // already on the smallest variant. "app" is no longer the floor —
  // "none" is, but we don't suggest "none" via this path because we'd
  // need to know that the workbook truly references zero symbols,
  // which the no-heavy-symbols check above already implies; an author
  // who explicitly picked "app" / "minimal" / "default" probably
  // wants wasm available even if the current bundle happens to not
  // call into it.
  if (variant !== "app" && variant !== "none") {
...
/**
 * Pre-scan the project source tree to pick the smallest variant the
 * workbook actually needs, BEFORE the bundle is built. Called by the
 * workbookInline plugin when `config.wasmVariant` is undefined so
 * authors get optimal bundles for free.
 *
 * Approach mirrors the post-bundle check: read every variant's
 * export universe, scan source files for property accesses matching
 * heavy symbols, return the smallest variant whose exports cover
 * everything used. False positives only push the variant LARGER —
 * never break the build.
 *
 * Source-tree scan rather than bundle scan because the bundle hasn't
 * been built yet at variant-pick time, and we don't want to bundle
 * twice. The trade-off: dead-code imports inflate the picked variant
 * (same trade-off the post-bundle check already accepts).
 *
 * @param {object} args
 * @param {string} args.runtimeDir Absolute path to packages/runtime-wasm.
 * @param {string} args.projectRoot Absolute path to the workbook project.
 * @returns {Promise<{variant: "none"|"app"|"minimal"|"default", scanned: number, usedSymbols: number, symbols: Set<string>}>}
 */
export async function pickVariantFromSource({ runtimeDir, projectRoot }) {
  const universe = await readVariantExports(runtimeDir, "default");
  if (!universe) {
    /* Can't read universe — fall back to "default" so the build
     * doesn't fail just because pkg/ wasn't generated locally. */
    return { variant: "default", scanned: 0, usedSymbols: 0, symbols: new Set() };
  }

  /* Walk the project's source tree, concatenate everything that
   * could legitimately reference a wasm symbol. Skip build outputs,
  ...
  const SOURCE_EXTS = new Set([".svelte", ".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs", ".svx"]);
  let scanned = 0;
  const blob = [];
  ...
  const usedSymbols = scanForUsage(blob.join("\n"), universe);
  if (usedSymbols.size === 0) {
    return { variant: "none", scanned, usedSymbols: 0, symbols: new Set() };
  }

  /* Find the smallest variant whose exports cover every used symbol. */
  for (const v of VARIANTS_ASCENDING) {
    if (v === "none") continue;
    const exp = await readVariantExports(runtimeDir, v);
    if (!exp) continue;
    ...
    for (const sym of usedSymbols) {
      if (!exp.has(sym)) { covers = false; break; }
    }
    if (covers) {
      return { variant: v, scanned, usedSymbols: usedSymbols.size, symbols: usedSymbols };
    }
  }
  /* Fallback — shouldn't hit unless the universe is broken. */
  return { variant: "default", scanned, usedSymbols: usedSymbols.size, symbols: usedSymbols };
}
