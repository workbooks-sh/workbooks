  ...
  switch (variant) {
    ...
    default:
      throw new Error(
        ...
        `Use "default", "minimal", "app", or "none".`,
      );
  }
}

export async function resolveRuntime(opts = {}) {
  // "none" — workbook has zero wasm.* surface usage. Skip the runtime
  // resolve entirely; the inline plugin will omit wasm-b64,
  // bindgen-src, and runtime-bundle-src tags.
  if (opts.variant === "none") {
    return { skipInline: true };
  }
