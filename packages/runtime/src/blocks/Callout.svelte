<script lang="ts">
  ...
  /* Inline markdown so agent-emitted `**bold**`, `` `code` ``, and
   * links render rather than leaking literal markup. Block-level
   * constructs (headings, lists) stay opt-in via the markdown kind. */
  ...
  const textHtml = $derived(inlineMd(block.text ?? ""));
...
>
  ...
  <div class="flex flex-1 flex-col gap-1 {palette.text}">
    ...
    {/if}
    <p class="text-[13px] leading-relaxed">{@html textHtml}</p>
  </div>
</aside>
