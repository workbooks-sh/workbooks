<script lang="ts">
  ...
  // Defensive: agent runners occasionally emit blocks whose `text`
  // is undefined (early model drafts, malformed render({block}) calls
  // before system prompts were tightened). Without this fallback the
  // tokenize() src.replace(...) call crashes the whole chat-page mount
  // for any session that contains one bad block.
