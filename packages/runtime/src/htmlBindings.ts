// shape-drift-ok: parses the documented "context-from" attribute set
// (notebook|document|deck), where "deck" is the shape-skill-facing alias
// for presentation. Not a workbook type literal.
/**
...
import { runPortableAgent, type AgentRunEvent } from "./agentRuntime";
import { createWorkbookBashTool } from "./agentBashTool";
import { sanitizeSvg } from "./util/sanitize";
import { installStageClient } from "./stage/autowire";
import {
...
/**
 * Declarative agent surface. Carries both legacy fields (id/reads,
 * <wb-tool ref="cellId">) and the formal authoring surface the four
 * shape skills document:
 *
 *   <wb-agent
 *     name="memo-assistant"           // alias of id
 *     surface="side-panel|full|inline"
 *     system="..."                     // alias of <wb-system>
 *     model="anthropic/claude-haiku-4.5"
 *     context-from="notebook|document|deck"
 *     emits="markdown,code,cell">
 *     <wb-tool name="search_memo"/>
 *     <wb-tool name="cite_paragraph"/>
 *   </wb-agent>
 *
 * `tools` carries the legacy cell-bound refs; `toolNames` carries
 * the named-tool filter applied to the host's createWorkbookAgentTools
 * surface. The two paths coexist — a host that wires both gets the
 * union.
 */
interface AgentSpec {
  id: string;
  ...
  toolNames: string[];
  surface?: "side-panel" | "full" | "inline";
  contextFrom?: "notebook" | "document" | "deck";
  emits: string[];
}
...
function parseSurface(raw: string | null): AgentSpec["surface"] {
  if (raw === "side-panel" || raw === "full" || raw === "inline") return raw;
  return undefined;
}

function parseContextFrom(raw: string | null): AgentSpec["contextFrom"] {
  if (raw === "notebook" || raw === "document" || raw === "deck") return raw;
  return undefined;
}
...
const ALLOWED_LANGUAGES = new Set<CellLanguage>([
  ...
  "wasm-fn",
  "chat",
  // "agent" is dispatched via the built-in agent cell executor (see
  // agentElement.ts). It's not a wasmBridge CellLanguage — the runtime
  // routes it through customCellRegistry. Listed here so the parser
  // accepts <wb-cell language="agent"> declarations.
  "agent",
]);
...
export function parseWorkbookHtml(root: Element): WorkbookHtmlSpec {
  ...
  // Agents. `name` is the documented authoring attribute; `id` is
  // accepted as a legacy alias. Either resolves to AgentSpec.id.
  for (const el of root.querySelectorAll("wb-agent")) {
    if (agents.length >= MAX_AGENTS) break;
    const id = validId(el.getAttribute("name") ?? el.getAttribute("id"));
    if (!id) continue;
    const rawModel = el.getAttribute("model") ?? "openai/gpt-4o-mini";
    if (!VALID_MODEL.test(rawModel)) continue;
    const model = rawModel;
    const reads = validIdList(el.getAttribute("reads"));
    // System prompt: attribute first (documented), <wb-system> child as fallback.
    const systemEl = el.querySelector("wb-system");
    const rawSystem =
      el.getAttribute("system") ?? systemEl?.textContent?.trim() ?? "";
    const systemPrompt = clipString(rawSystem, MAX_SYSTEM_PROMPT_BYTES);
    const toolEls = [...el.querySelectorAll("wb-tool")].slice(0, MAX_TOOLS_PER_AGENT);
    const tools = toolEls
      .map((t) => validId(t.getAttribute("ref")))
      .filter((s): s is string => Boolean(s));
    const toolNames = toolEls
      .map((t) => validId(t.getAttribute("name")))
      .filter((s): s is string => Boolean(s));
    const surface = parseSurface(el.getAttribute("surface"));
    const contextFrom = parseContextFrom(el.getAttribute("context-from"));
    const emits = validIdList(el.getAttribute("emits"));
    agents.push({
      id, model, systemPrompt, reads, tools, toolNames, surface, contextFrom, emits,
    });
  }
...
export interface MountOptions {
  ...
  /**
   * Shared agent runtime event sink. Browser workbook agents emit the
   * same event names and payload family as Studio sandbox agents so
   * hosts can render one transcript model regardless of adapter.
   */
  onAgentEvent?: (event: AgentRunEvent & { agentId: string }) => void;
}
...
export async function mountHtmlWorkbook(opts: MountOptions): Promise<{
  ...
  // Per-mount → module-global lookup chain (core-0id.11). The "agent"
  // cell language is auto-resolved to the built-in agent executor when
  // no host override is registered, so skill-shaped workbooks that use
  // `<wb-cell language="agent">` work zero-config. Hosts can override
  // by registering their own "agent" executor on the per-mount or
  // module-global registry.
  const localRegistry = opts.cellRegistry;
  let builtinAgentExecutor: CustomCellExecutor | undefined;
  const lookupCustom = (language: string): CustomCellExecutor | undefined => {
    const found = localRegistry?.get(language) ?? customCellRegistry.get(language);
    if (found) return found;
    if (language === "agent") {
      builtinAgentExecutor ??= createBuiltinAgentCellExecutor();
      return builtinAgentExecutor;
    }
    return undefined;
  };
  ...
  if (typeof window !== "undefined") {
    ...
    // Auto-wire the stage client. When the workbook is hosted in a
    // stage iframe, this binds the wrapped workbook's Y.doc to the
    // host's canonical doc; standalone, it returns a local-only doc
    // with no transport. Either way, `window.__wbRuntime.stage` (and
    // legacy `.playground`) is reachable from user code without anyone
    // having to import `connectToStage` explicitly.
    try {
      installStageClient();
    } catch (err) {
  ...
  const buildAgentTools = (agent: AgentSpec): AgentTool[] => {
    ...
    // Named-tool filter — <wb-tool name="..."> children resolve against
    // the host-provided opts.agentTools (and the default bash tool's
    // name). When toolNames is non-empty, only matching tools are
    // exposed to this agent. When empty, the agent gets the full
    // host-provided surface — the existing behavior.
    if (agent.toolNames.length > 0) {
      const wanted = new Set(agent.toolNames);
      const pool: AgentTool[] = [];
      ...
      if (extraTools) pool.push(...extraTools);
      return pool.filter((t) => wanted.has(t.definition.name));
    }
  ...
  for (const chat of doc.querySelectorAll("wb-chat")) {
    bindChatElement(chat as HTMLElement, ctx, spec, buildAgentTools, opts.onAgentEvent);
  }

  // Wire <wb-agent> trigger buttons (manual run for now).
  for (const agentEl of doc.querySelectorAll("wb-agent")) {
    bindAgentElement(agentEl as HTMLElement, ctx, spec, buildAgentTools, opts.onAgentEvent);
  }
...
// ----------------------------------------------------------------------
// Built-in agent cell executor — lazy default for <wb-cell language="agent">
// when the host hasn't registered an override. Kept inline (not pulled
// from agentElement.ts) to avoid a circular import; agentElement.ts is
// the canonical surface for host overrides.
// ----------------------------------------------------------------------

const DEFAULT_AGENT_CELL_MODEL = "anthropic/claude-haiku-4.5";

function createBuiltinAgentCellExecutor(): CustomCellExecutor {
  return {
    async execute({ source, params, cellId, ctx }) {
      const llmClient = ctx.llmClient;
      if (!llmClient) {
        return [{ kind: "error", message: `agent cell '${cellId}': no llmClient configured` }];
      }
      const meta = readAgentCellMetadata(cellId);
      const prompt = source.trim();
      if (!prompt) {
        return [{ kind: "error", message: `agent cell '${cellId}': empty prompt` }];
      }
      const contextLines = Object.entries(params)
        .filter(([, v]) => v != null && v !== "")
      ...
      try {
        const result = await runPortableAgent({
          ...
          runtime: {
            ...
            reason: "built-in agent cell executor",
          },
        });
...
function readAgentCellMetadata(cellId: string): { model: string; systemPrompt: string } {
  if (typeof document === "undefined") {
    return { model: DEFAULT_AGENT_CELL_MODEL, systemPrompt: "" };
  }
  const el = document.querySelector(
    `wb-cell[id="${CSS.escape(cellId)}"]`,
  ) as HTMLElement | null;
  if (!el) return { model: DEFAULT_AGENT_CELL_MODEL, systemPrompt: "" };
  const model = el.getAttribute("model") ?? DEFAULT_AGENT_CELL_MODEL;
...
function bindAgentElement(
  ...
  // `name` is the documented authoring attribute; `id` is the legacy alias.
  // Either resolves to AgentSpec.id (parser normalizes).
  const lookupId = el.getAttribute("name") ?? el.getAttribute("id");
  if (!lookupId) return;
  const agent = spec.agents.find((a) => a.id === lookupId);
  if (!agent) return;

  // Expose surface/context/emits as data-* on the mounted element so
  // consuming workbook CSS can position the chat panel without the
  // runtime imposing a layout. Skills lean on `data-wb-agent-surface`.
  if (agent.surface) el.dataset.wbAgentSurface = agent.surface;
  if (agent.contextFrom) el.dataset.wbAgentContextFrom = agent.contextFrom;
  if (agent.emits.length > 0) el.dataset.wbAgentEmits = agent.emits.join(",");
  ...
  if (el.hasAttribute("auto") || el.hasAttribute("trigger") === false) {
    runAgentOnce(el, ctx, agent, buildTools(agent), onAgentEvent).catch((err) =>
...
async function runAgentOnce(
  ...
  const result = await runPortableAgent({
    ...
    runtime: {
      target: "browser-js",
      adapter: "workbook-html",
      reason: "Selected browser-js because this agent is running inside a user-present workbook tab.",
    },
    emit: (event) => onAgentEvent?.({ ...event, agentId: agent.id }),
  });
