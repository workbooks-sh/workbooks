  ...
  onMount(async () => {
    try {
      ...
      if (typeof window !== "undefined") {
        ...
        // Auto-wire the stage client — same rationale as in
        // mountHtmlWorkbook. Local-only when the workbook isn't
        // embedded in a stage iframe; the doc lands on
        // window.__wbRuntime.stage (with .playground alias) either way.
        try {
          installStageClient();
        } catch (err) {
