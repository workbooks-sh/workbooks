...
// Canonical workbook SHAPES (document / notebook / spa / presentation).
// Single source of truth for the 4-shape model — CLI, Studio, demo
// fixtures, anywhere else that branches on shape imports from here.
// See shapes.ts for the rationale + CI drift-lint reference.
export {
  WORKBOOK_SHAPES,
  SHAPE_DESCRIPTIONS,
  isWorkbookShape,
} from "./shapes.js";
export type { WorkbookShape, WorkbookShapeMeta } from "./shapes.js";
...
// spa workbooks. Plays CW XML timelines with HyperFrames HTML scenes.
// Render-to-MP4 is deferred; the chrome's "Render" button is a stub.
export {
  Theater,
  Composition,
  getTheaterContext,
...
export type { TheaterApi, CompositionHandle } from "./composition";

// Brand logos — bundled at build time into the workbook artifact,
// accessed at runtime via `wb.logos.<as>.dataUrl` (or .svg). See
// workbook.config.mjs > logos for the build-side declaration.
...
// Declarative agent surface — registers the "agent" cell language
// (<wb-cell language="agent">) and the inert <wb-agent>/<wb-tool>
// custom elements. The four shape skills (document/notebook/spa/
// presentation) document these as the authoring surface; this module
// is the bridge to the imperative runAgentLoop runtime.
export {
  createAgentCellExecutor,
  installAgentCellLanguage,
  installAgentElement,
} from "./agentElement";
export type { AgentCellOptions } from "./agentElement";

// Workbook → backend integration client. Lets workbooks call connected
// toolkits (Gmail, GitHub, …) via our backend, without ever touching
// raw OAuth tokens. See connections.ts for the API surface.
export { connections, ConnectionsError } from "./connections";
export type { ConnectionsClient } from "./connections";

// Floater — runtime-side primitive for surfacing "this workbook needs
// something" (missing creds, unconnected integrations) consistently
// across shapes. Plain-DOM widget when standalone; forwards to host
// via postMessage when embedded so Studio shows a consolidated
// surface. Connection modules push items via floater.add().
export { floater } from "./floater";
export type { FloaterItem, FloaterTone } from "./floater";

// Cross-workbook tool RPC. Workbooks register handlers for their
// declared tools; calls between co-resident workbooks route through
// the host (Studio) bus instead of the broker MCP endpoint. Standalone
// workbooks throw WbRpcNoRoute on calls to non-local tools unless a
// fallback transport is configured. wb-1ru.5 + wb-1ru.6.
export { rpc, WbRpcError, WbRpcNoRoute } from "./rpc";
export type { RpcHandler, RpcToolDecl } from "./rpc";
export { brokerMcpFallback } from "./rpc/brokerFallback";
export type { BrokerFallbackOptions } from "./rpc/brokerFallback";
...
// Content-addressed compile cache for WebAssembly modules. Consumers
// feed the returned Module to wasm-bindgen's init to skip compile on
// returning visits.
export { getCachedModule, clearCache, isDegraded } from "./wasmCache";
export type { WasmCacheOptions } from "./wasmCache";
...
export { runPortableAgent, agentRunEvent } from "./agentRuntime";
export type {
  AgentRunEvent,
  AgentRunEventKind,
  AgentRuntimeDescriptor,
  AgentRuntimeTarget,
  RunPortableAgentOptions,
} from "./agentRuntime";
export type {
  AgentTool,
  AgentLoopOptions,
