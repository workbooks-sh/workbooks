# ugc-pro-script-writer

A `type:"agent"` workbook. Unlike an `spa` (a static interface the reader interacts with directly), an agent is a server-side LLM loop the reader messages, with drafts streaming into a `stage` component the reader watches. The `.html` artifact is a catalog page; the actual loop runs in Studio after publish.
...
- `src/components/stage.js` — the agent's stage component, matching the `(target, props, emit) => unmount` contract. The agent pushes drafts into this via `render({component:"stage", props:{draft}})`; `props.draft` is the current text. Replace with whatever the agent's outputs need (table, slides, code editor, etc.).
...
workbook build                       # produces dist/ugc-pro-script-writer.html
